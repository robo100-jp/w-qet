...
**カタログとは役割が違う。** カタログは176個ぜんぶを載せて「形から引く」もの。
...
# 正式な名称は docs/カタログ.md と docs/採録状況.md にある。
GROUPS = [
    ...
    ("始動器", [("07-14-01", "電動機始動器"), ("07-14-05", "直結始動器（可逆）"),
               ("07-14-06", "スターデルタ")]),
...
TITLE = "JIS C 0617 第7部 図記号 早見表"
SUB = ("w-qet — JIS C 0617（＝IEC 60617）を参考に描き起こしたもの。"
       "全176個の一覧は docs/カタログ.md")
...
def main():
    ...
    out.append('<text x="%g" y="%g" font-family="%s" font-size="2.5"'
               ' fill="#555">%s</text>' % (MARGIN, y, S.CAPFONT, escape(SUB)))
    y += 3.2
