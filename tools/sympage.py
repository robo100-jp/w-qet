...
def page(path, notes):
    ...
    w('<h2>一般的な名称<span class="pencil">手で書く欄</span></h2>')
    w('<p class="rule">この図記号が指す部品を、ふだん何と呼ぶか。'
      "現場・部品表・カタログでの言い方。<b>規格の名称とは別。</b>"
      "規格が「安全開離機能接点」でも、通じるのは「ミラー接点」。"
      "<b>探すときはこの言葉で探すので、目次の絞り込みもここを見る。</b><br>"
      "<b>名前だけを書く。</b>どういう部品かの説明は次の欄へ。"
      "ここは <code>.elmt</code> の名称（QET の部品パネルに出る名前）の元になる。</p>")
    w(hand("names", "一般的な名称・部品表に書く名前・カタログでの言い方"))

    w('<h2>部品の説明<span class="pencil">手で書く欄</span></h2>')
    w('<p class="rule">どういう部品か、何をするものか。'
      "<b>名前ではないものはここに書く。</b>"
      "「部品ではない。他の記号に重ねて使う印」「旧図記号。いまは○○を使う」なども。<br>"
      "上の欄に説明を混ぜると、<b>そのまま QET の部品名になってしまう。</b></p>")
    w(hand("desc", "どういう部品か、何をするものか"))
