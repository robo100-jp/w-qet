...
**JIS C 0617 第7部（開閉装置，制御装置及び保護装置）と
第8部（計器，ランプ及び信号装置）を、どちらも全部描き起こしました。**

| 内容 | 状態 |
|---|---|
| `elements/` 図記号 | 第7部・第8部 ＋ それぞれの旧図記号（附属書A）＋ 作図用部品 |
| `tools/` 作図・検証ツール（Python） | あり |
| `.claude/skills/` Claude Code 用スキル | あり |

**個数はここに書きません。**`py -3 tools/status.py`（第8部は `--part 8`）が
`elements/` を走査して数え直し、[採録状況](docs/採録状況.md) を作り直します。
...
> `07-A` `08-A` は規格の附属書A に載っている**既に削除された図記号**です。
